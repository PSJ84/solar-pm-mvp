...
- 로컬 환경에서 dev 서버를 상시 실행하지 않는다.
- Web 프론트는 Vercel 배포본에서 확인한다.
- API 서버는 Railway 배포본에서 확인한다.
- DB는 Supabase(Postgres) 사용.
- 스키마 변경은 “배포 환경에서 깨지지 않도록” Prisma migration 기준으로 관리한다.
- 기능 검증은 기본적으로 배포 URL 기준으로 진행한다.
